...
export interface PodcastDocListItemProps {
  docId: string;
  groupId: string;
}

export const PodcastDocListItem = ({
  docId,
  groupId,
}: PodcastDocListItemProps) => {
...
// 列表视图组件
const ListViewDoc = ({ docId }: PodcastDocListItemProps) => {
  const t = useI18n();
...
// 卡片视图组件
const CardViewDoc = ({ docId }: PodcastDocListItemProps) => {
  const t = useI18n();
  const contextValue = useContext(DocExplorerContext);
  const selectMode = useLiveData(contextValue.selectMode$);
  const docsService = useService(DocsService);
